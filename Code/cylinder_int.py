...
class cylinder_int:
    ...
    def directivity(self,phis,N=75):
        try:
...
plt.polar(phis,np.abs(hard_dirs))


#%%
plt.polar(phis,np.abs(soft_dirs))     

#%%
ks = np.linspace(0.01,10,400)
inters = [cylinder_int(k,1,np.pi/2, np.pi,is_hard=True) for k in ks]  
dirs = [inter.directivity(np.pi) for inter in inters]

plt.plot(ks,np.abs(dirs))
